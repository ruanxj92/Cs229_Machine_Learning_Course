...
$$
其中$\eta$称为分布的**自然参数**（也叫作**典型参数**）T(y)是充分统计量（对于我们考虑的分布，经常有T(y)=y），$a(\eta)$是**对数划分函数**. $e^{-a(\eta)}$的数量在归一化常数中起到非常重要的作用,他能保证$p(y;\eta)$的分布的和除以y得1.
...
\end{align}
$$
因此，自然参数由$\eta=log(\phi/(1-\phi))$给出。有趣的是，如果我们用$\eta$解出$\phi$的表达式，会得到$\phi\frac{1}{1+e^{-\eta}}$。这就是我们熟悉的sigmoid函数表达####8 指数族
为了适应GLM，我们先定义指数族分布。当满足下式时，我们说一类分布是指数族分布：
$$
p(y;\eta)=b(y)exp(\eta^TT(y)-a(\eta))
$$
其中$\eta$称为分布的**自然参数**（也叫作**典型参数**）T(y)是充分统计量（对于我们考虑的分布，经常有T(y)=y），$a(\eta)$是**对数划分函数**. $e^{-a(\eta)}$的数量在归一化常数中起到非常重要的作用,他能保证$$的分布的和除以y得1.
T, a, b的选择定义了由$$参数化的一族分布,当我们改变$$的值,我们能获得这个分布族内的不同分布.
现在我们将说明伯努利分布和高斯分布是指数分布族内的两个例子. 均值为$$的伯努利分布计做$$,其定义了$$的分布故$$.当改变$$的值,我们得到不同均值的伯努利分布. 现在说明的这类伯努利分布是改变$$得到的一类指数族. 例如对于特定的$$那方程(6)就正好变成伯努利分布类型.
[^footnote5]:本章节的展示材料灵感来源于 Michael I.Jordan, *Learning in graphical models*(未出版书稿),以及McCullagh and Nelder, *Generalized Linear Models*(第二版)
...
#####9.1 普通最小二乘
#####9.2 logistic 回归
#####9.3 softmax 回归式！这就又回到了推导出logistic函数是一种GLM。为了完成伯努利分布的指数族分布公式，我们有:
$$
\begin{align}
T(y)&=y\\
a(\eta)&=-log(1-\phi)
...
现在让我们考虑高斯分布。回到我们推导线性回归的时候，$\sigma^2$的值对于最后$\theta 和h_{\theta}(x)$的取值没有影响。为了简化推导过程，令$\sigma^2=1$。我们有：
$$
\begin{align}
